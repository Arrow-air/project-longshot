...
- `kicad/libs/BMSJ.pretty/` — project-local footprints imported from the source repository.
- `firmware/vesc_bms_hw/` — VESC BMS hardware configuration renamed to `hw_bmsj`.
- `docs/`, `images/`, and `manufacturing/` — placeholders for supporting design notes, renders, and fab outputs.

## Overview

BMSJ is a modular BMS supporting up to **24 series cells** (24S) with:
...
BMSJ uses the [VESC BMS firmware](https://github.com/vedderb/vesc_bms_fw) which provides:
...
# Copy BMSJ hardware config
cp /path/to/project-longshot/engineering/electronics/pcbs/BMSJ/firmware/vesc_bms_hw/hw_bmsj.h hwconf/
cp /path/to/project-longshot/engineering/electronics/pcbs/BMSJ/firmware/vesc_bms_hw/hw_bmsj.c hwconf/
...
The BMSJ hardware config is in `firmware/vesc_bms_hw/`:

```
firmware/vesc_bms_hw/
├── hw_bmsj.h     # Pin definitions, parameters
└── hw_bmsj.c     # Initialization, INA226 driver
```

Key parameters in `hw_bmsj.h`:
...
2. Open `kicad/BMSJ.kicad_pro` in KiCad
